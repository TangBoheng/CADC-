...
{printf("print_detector_detections");
    int i, j;
    for (i = 0; i < total; ++i) {
...
{printf("print_imagenet_detections");
    int i, j;
    for (i = 0; i < total; ++i) {
        float xmin = dets[i].bbox.x - dets[i].bbox.w / 2.;
        float xmax = dets[i].bbox.x + dets[i].bbox.w / 2.;
...
{printf("test_detector\n");
    ...
    while(1){
        ...
        int nboxes = 0;
        detection *dets = get_network_boxes(&net, im.w, im.h, thresh, hier_thresh, 0, 1, &nboxes, letterbox);//取得roi
        if (nms) do_nms_sort(dets, nboxes, l.classes, nms);
...
{   printf("run_detector\n");
    ...
    int cam_indexl = find_int_arg(argc, argv, "-c", 0);//相机地址
	int cam_indexr = find_int_arg(argc, argv, "-c", 1);//相机地址
   ...
   char *gpu_list = find_char_arg(argc, argv, "-gpus", 0);
    ...
    if(0==strcmp(argv[2], "test")) test_detector(datacfg, cfg, weights, filename, thresh, hier_thresh, dont_show, ext_output, save_labels);
        ...
        char *name_list = option_find_str(options, "names", "data\\720.names");
        char **names = get_labels(name_list);
        if(filename)
            if(strlen(filename) > 0)
                if (filename[strlen(filename) - 1] == 0x0d) filename[strlen(filename) - 1] = 0;
        demo(cfg, weights, thresh, hier_thresh, cam_indexl,cam_indexr, filename, names, classes, frame_skip, prefix, out_filename,
